...
#<---------------USERS GET/POST REQUEST--------------->
      ...
      if userExists
        ...
        [ {:message => "User Exists!", :user => userExists}.to_json]]
      else    
        new_user = User.create(username: data["username"])
        return [200, { 'Content-Type' => 'application/json' }, 
        [ {:message => "Sign-in Completed", :user => new_user}.to_json]]
      end
...
#<----------------BOARDS GET REQUEST----------------->
    ...
    return [200, { 'Content-Type' => 'application/json' }, [ {:board => boards}.to_json]]
#<----------------BOARDS POST REQUEST---------------->
