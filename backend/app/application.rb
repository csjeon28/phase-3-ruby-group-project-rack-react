...
#<-----------USERS GET REQUEST------------->
      ...
      users = User.all.map do |user| {username: user.username, board_id: user.board_id}
      end
...
#<-----------USERS POST REQUEST------------->
      ...
      if userExists
        ...
        create_user = {username: data["username"], board_id: data["board_id"]}
        return [200, { 'Content-Type' => 'application/json' }, 
...
#<-----------BOARDS GET REQUEST------------->
      ...
      boards = Board.all.map do |board| {name: board.name, user_id: board.user_id}
      end
...
#<-----------BOARDS POST REQUEST------------->
      ...
      create_board = {id: board.id, name: board.name}
      # Board.create(name: data["name"])
      # create_board = {name: data["name"]}
      return [200, { 'Content-Type' => 'application/json' }, 
      [ { :data => {:message => "Board created", :board => create_board}}.to_json]]
